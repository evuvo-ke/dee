...
class BorrowersViaGroupImport implements ToCollection, WithHeadingRow, WithChunkReading, ShouldQueue
{
    ...
    public function __construct($groupID,$user=null)
    {
    ...
    {
        $group = BorrowerGroup::find($this->groupID);
        foreach ($rows as $row) {
            ...
            if (!$data) {
                ...
                $borrower->loan_officer_id = $group->loan_officer_id;
                if (strtolower($row['gender']) === 'male' || strtolower($row['gender']) === 'm') {
                    $borrower->title = 'Mr';
                } else {
                    $borrower->title = 'Ms';
                ...
                $member->borrower_group_id = $this->groupID;
        ...
        Notification::route('mail', $this->email)->notify(new BorrowersImportFinishedNotification($this->results,$this->user));
