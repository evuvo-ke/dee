<div class="grid-stack-item loan_statistics"
    ...
    if (!empty($date_range)) {
        ...
        $start_date = $exploded_date[0];
        $end_date = $exploded_date[1];
    }
    $loans_disbursed = \Modules\Loan\Entities\Loan::where('status', 'active')
        ->when($start_date && $end_date, function ($query) use ($start_date, $end_date) {
            $query->whereBetween('loans.disbursed_on_date', [$start_date, $end_date]);
        })
        ->sum('principal');
    $repayments = \Modules\Loan\Entities\LoanTransaction::where('reversed', 0)
            ...
            <div class="col-md-3">
                <div class="info-box">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">{{number_format($loans_disbursed,2)}}</span>
                    </div>
                    <!-- /.info-box-content -->
                </div>
            </div>
            <div class="col-md-3">
                <div class="info-box">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">{{number_format($repayments,2)}}</span>
                    </div>
                    <!-- /.info-box-content -->
                </div>
            </div>
            <?php
            $loans = \Modules\Loan\Entities\Loan::with('repayment_schedules')
                ->where('status', 'active')
                ->when($start_date && $end_date, function ($query) use ($start_date, $end_date) {
                    $query->whereBetween('loans.disbursed_on_date', [$start_date, $end_date]);
                })
